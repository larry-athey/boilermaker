...
Windows users will also need to download and install the [CP210x driver](https://www.silabs.com/developer-tools/usb-to-uart-bridge-vcp-drivers?tab=downloads) so the Boilermaker will appear as a serial port that your terminal program can connect to. Linux and Mac OS users need not bother with this since the driver is already part of the kernel tree. This driver is actually used with numerous devices, not just ESP32 micro controllers.
