...
The examples below use the **Temperature Cruise** mode. This is the same temperature control method that's used in my [Airhead](https://github.com/larry-athey/airhead) upgrade for Air Stills. It's basically controlled chaos, it provides a rapid high thermal velocity temperature rise with a very limited risk of scorching because the temperature rise is linear, not just an instant constant blast of high power until the target temperature is reached. This mode is intended for pot still distillations or to preheat a **Brewing/Fermentation** run.
