...
_**NOTE:** The Boilermaker uses a fully text mode user interface rather than a fancy graphics based interface because it doesn't rely on a centralized server like other mobile apps do. Sorry, no fancy dial/slider controls and sound effects here, I'm not a mobile app developer._
...
**Current Mode:** This will either say "Constant Power", "Temperature Cruise", or "Brewing/Fermentation". **Constant Power** will run the unit like any other SCR power controller. **Temperature Cruise** will honor a target temperature and manage the power in order to maintain that temperature using high thermal velocity, this is the same temperature management method used in the [Airhead](https://github.com/larry-athey/airhead) upgrade for Air Stills. **Brewing/Fermentation** is a strict, yet adjustable, PID controller that is geared by default for slow and steady temperature management, not distillation due to its reduced thermal velocity.

**Target Temp:** As the name implies, this is the target temperature that you want to shoot for. Everything is done in Celcius at the hardware level, so you need to enter your choice in Celcius here. The upper limit of 260C is 500F, which will require you to be using a thermocouple instead of a DS18B20. This will allow you to use a Boilermaker to control an electric grill or other high temperature appliance. I actually use a Boilermaker with a DS18B20 to run the cheapest bottom of the barrel [Masterbuilt steam smoker](https://www.amazon.com/gp/product/B07VRJQXGL/) you can buy. Cheap way to have a WiFi enabled and accurate smoker!
...
In **Constant Power** mode, you naturally have a progressive temperature ramp which will continue to rise. This feature allows you to create that ramp with an actual end point that no longer continues to rise. However, unlike **Constant Power** mode, the thermal velocity is reduced which will slow down distillation. This isn't always a bad thing, sometimes you need a less aggressive boil in pot still runs in order to tame down unwanted flavors and to avoid scorching if distilling on the grain.
