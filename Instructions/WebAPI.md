...
_**NOTE:** This API only returns JSON objects for success or failure when setting values. Requested values are returned as-is, plain text, no formatting or encoding. The ESP32 has limited code space and storing text strings eats up more space than plain old evaluation code. If we were dealing with unlimited code space like you have on any server running PHP, I'd be telling a different story here._
