...
**/?set-correctionfactor=** - Set a new temperature sensor correction factor. The value passed here is a temperature in Celcius and must be a whole or floating point number between -15 and 15. If you are using a sensor mounted outside of the boiler, such as when the sensor is mounted to the bottom like an Air Still. You will need to use a positive correction factor since it will be reading lower than the actual contents of the boiler. For example, the Airhead uses a +5 correction factor in most cases.
