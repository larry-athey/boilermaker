...
The icing on the cake with this PWM and SSR combination is that it doesn't contribute to premature heating element failure as with high-speed on/off switching of an SCR controller. SCR phase-angle control generates harmonics _(distortions in the AC sine wave)_ which can lead to uneven current distribution, vibration, and stress on a heating element.
